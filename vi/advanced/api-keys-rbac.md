...
Nếu `GOCLAW_GATEWAY_TOKEN` trống (không cấu hình gateway token), tất cả các request — kể cả không có xác thực — đều được cấp quyền `RoleOperator` tự động. Điều này cho phép các triển khai self-hosted hoạt động mà không cần xác thực chặt chẽ. Khi đã cấu hình token, tất cả request phải cung cấp credentials hợp lệ, nếu không sẽ nhận `401 Unauthorized`.
